...
def ablation_graph(batch):
    reac_moles, prod_moles = [], []
    ...
    return graph_col_fn(reac_moles), graph_col_fn(prod_moles), \
        torch.LongTensor(reactant_pairs), torch.LongTensor(product_pairs)


def uspto_condition_ablation(batch):
    xinfo = ablation_graph([x[0] for x in batch])
    labels = torch.LongTensor([x[1] for x in batch])
    label_types = torch.LongTensor([[0, 1, 1, 2]] * labels.shape[0])
    return xinfo + (label_types, labels, len(batch))


def uspto_500mt_ablation(batch):
    xinfo = ablation_graph([x[0] for x in batch])
    labels = [x[1] for x in batch]
    return xinfo + (labels, len(batch))
